...
for(let indice = 0; indice < arrayWhiteTagsChildren.length; indice++) {
    const current= arrayWhiteTagsChildren[indice]
    const currentHeight = current.clientHeight
    if(prev + currentHeight >= limitPage) {
        current.style.color = 'orange'
        ArraytagsBr.forEach(tag => {
            arrayWhiteTagsChildren.splice(indice, 0, document.createElement('br'))
    })
        prev = 0
...
arrayWhiteTagsChildren.forEach( element => {
    initialHTML.appendChild(element) 
})

    return true
}
console.log('Funcionou?', pdfWithPageBreak('tbody', 100) ? 'SIM' : 'NÃO')
